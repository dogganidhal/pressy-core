...
export class OrderController extends BaseController {
  ...
  private _orderRepository: IOrderRepository =
    RepositoryFactory.instance.orderRepository;
  private _invoiceRepository: IInvoiceRepository =
    RepositoryFactory.instance.invoiceRepository;
  private _articleRepository: IArticleRepository =
    RepositoryFactory.instance.articleRepository;
  ...
  public async createInvoice(
    @JSONBody(TerminateOrderRequest) request: TerminateOrderRequest
  ): Promise<InvoiceDto> {
  ...
  public async getTodayOrders(): Promise<OrderDto[]> {
    ...
    let orders = await this._orderRepository.getTodayOrdersByLaundryPartner(
      laundrer.laundryPartner
    );
  ...
  public async getOrderInfo(@PathParam("id") id: number): Promise<OrderDto> {
    ...
    if (!order) throw new exception.OrderNotFoundException(id);
